# ================================================================
# FES-MADM II R/Shiny Application
# Version: 1.1.0 - Professional UI build, export-clean revision
#
# Purpose:
#   Interactive implementation of the FES-MADM II fuzzy entropy-synergy
#   multi-attribute decision-making framework.
#
# Important implementation note:
#   The computational core of the original application has NOT been
#   modified in this version. The changes introduced here concern only
#   presentation, layout, styling, plot appearance, user-interface readability,
#   and clean Excel export headers.
#
# Export-clean revision:
#   The Excel download handler now assigns column headers explicitly in all
#   exported sheets. This prevents non-informative headers such as "...1" from
#   appearing in the output workbook and improves reproducibility packaging for
#   GitHub/Zenodo archival.
#
# Main UI refinements:
#   - Professional light visual theme.
#   - Improved header, navigation, tables, buttons, and panels.
#   - More readable ggplot outputs through a unified plotting theme.
#   - Cleaner summary/results presentation.
#
# Required packages:
#   shiny, shinythemes, readxl, writexl, ggplot2, dplyr, DT, gridExtra, reshape2
#
...
plot_theme_fes <- function(base_size = 13) {
  theme_minimal(base_size = base_size) +
    theme(
      plot.title = element_text(face = "bold", colour = APP_COLORS$primary, size = base_size + 2),
      plot.subtitle = element_text(colour = APP_COLORS$muted, size = base_size),
      axis.title = element_text(face = "bold", colour = APP_COLORS$text),
      axis.text = element_text(colour = APP_COLORS$text),
      panel.grid.minor = element_blank(),
      panel.grid.major.x = element_blank(),
      legend.position = "bottom",
      legend.title = element_text(face = "bold"),
      plot.background = element_rect(fill = APP_COLORS$surface, colour = NA),
      panel.background = element_rect(fill = APP_COLORS$surface, colour = NA),
...
ui <- fluidPage(
  ...
  tags$head(
    tags$style(HTML("
      ...
      .app-hero {
        ...
        box-shadow: 0 8px 24px rgba(11, 57, 84, 0.18);
      }
  ...
  tags$div(
    class = "app-hero",
    tags$h1("FES-MADM II Decision Support Environment"),
    tags$p("Fuzzy Entropy–Synergy Multi-Attribute Decision-Making with α-cut sensitivity and entropy diagnostics")
  ),
...
server <- function(input, output, session) {
  ...
  output$plotAltScores <- renderPlot({
    ...
    ggplot(df, aes(x = reorder(Alternative, -Score), y = Score)) +
      geom_col(width = 0.72, fill = APP_COLORS$primary, colour = APP_COLORS$border, alpha = 0.95) +
      geom_text(aes(label = round3(Score)), vjust = -0.55, size = 3.8) +
  ...
  output$plotWeightsSBJ <- renderPlot({
    ...
    ggplot(df, aes(x = reorder(Criterion, -Weight), y = Weight)) +
      geom_col(width = 0.72, fill = APP_COLORS$primary, colour = APP_COLORS$border, alpha = 0.95) +
      geom_text(aes(label = round3(Weight)), vjust = -0.55, size = 3.8) +
  ...
  output$plotWeightsCompare <- renderPlot({
    ...
    if (nrow(df_melt_filtered) == 0) {
      ...
      ggplot(df_melt_filtered,
        ...
        geom_line(size = 1.05) +
        geom_point(size = 3.2) +
        geom_text(aes(label = round3(Weight)), vjust = -0.55, size = 3.8, colour = APP_COLORS$text) +
  ...
  output$plotEntropyMeasures <- renderPlot({
    ...
    ggplot(df, aes(x = Measure, y = Value)) +
      geom_col(width = 0.72, fill = APP_COLORS$primary, colour = APP_COLORS$border, alpha = 0.95) +
      geom_text(aes(label = Value), vjust = -0.55, size = 3.8) +
  ...
  output$downloadAll <- downloadHandler(
    ...
    content = function(file) {
      ...
      # Export helper: force clean and stable Excel headers.
      # This is intentionally limited to output formatting and does not affect
      # any FES-MADM II calculation.
      make_export_table <- function(id_name, id_values, lower, upper, crisp) {
        out <- data.frame(
          id_values,
          as.numeric(lower),
          as.numeric(upper),
        ...
        names(out) <- c(id_name, "Lower", "Upper", "Crisp")
        out
      }
      
      df_alts <- make_export_table(
        id_name   = "Alternative",
        id_values = paste0("Y", 1:N),
        lower     = r$altScoreLower,
        upper     = r$altScoreUpper,
        crisp     = r$altDef
      )
      
      df_sbj <- make_export_table(
        id_name   = "Criterion",
        id_values = paste0("X", 1:M),
        lower     = r$wLower,
        upper     = r$wUpper,
        crisp     = r$wDefSBJ
      )
      
      df_obj <- make_export_table(
        id_name   = "Criterion",
        id_values = paste0("X", 1:M),
        lower     = r$xObjLower,
        upper     = r$xObjUpper,
      ...
      df_entropy <- make_export_table(
        id_name   = "Measure",
        id_values = c("S(X,Y)", "S(X)", "S(Y)", "I(X;Y)", "S(Y|X)", "I(Y|X)"),
        lower     = c(r$SXY_L, r$SX_L, r$SY_L, r$IXY_L, r$SYX_L, infoProp_L),
        upper     = c(r$SXY_U, r$SX_U, r$SY_U, r$IXY_U, r$SYX_U, infoProp_U),
        crisp     = c(r$SXY_C, r$SX_C, r$SY_C, r$IXY_C, r$SYX_C, infoProp_C)
      )
      
      df_indices <- make_export_table(
        id_name   = "Index",
        id_values = c("NMI", "ADI", "CES", "CSF", "NMGI"),
        lower     = c(r$NMI_L, r$ADI_L, r$CES_L, r$CSF_L, r$NMGI_L),
        upper     = c(r$NMI_U, r$ADI_U, r$CES_U, r$CSF_U, r$NMGI_U),
        crisp     = c(r$NMI_C, r$ADI_C, r$CES_C, r$CSF_C, r$NMGI_C)
      )
      
      # Keep the original sheet structure, but export all sheets with clean
  ...
  output$summaryResults <- renderUI({
    ...
    tags$table(
      style = "width:100%; border-collapse:collapse; border: 1px solid #D8E2EA;",
      tags$tr(
        tags$th(colspan = 2,
                style = "text-align:left; padding:8px; border:1px solid #D8E2EA;
                         background-color:#0B3954; color:#FFFFFF; font-weight:bold; font-size:18px;",
                "Metric")
      ),
      tags$tr(
        tags$td(style="padding:8px; border:1px solid #D8E2EA;", strong("Selected α-cut")),
        tags$td(style="padding:8px; border:1px solid #D8E2EA;", sprintf("%.3f", alpha_val))
      ),
      tags$tr(
        tags$td(style="padding:8px; border:1px solid #D8E2EA;", strong("Optimal Alternative")),
        tags$td(style="padding:8px; border:1px solid #D8E2EA;",
                paste0(altStr, " (Crisp score p*ν: ", sprintf("%.3f", max(r$altDef)), ")"))
      ),
      tags$tr(
        tags$td(style="padding:8px; border:1px solid #D8E2EA;", strong("Most Significant Criterion (ICI)")),
        tags$td(style="padding:8px; border:1px solid #D8E2EA;",
      ...
      tags$tr(
        tags$td(colspan = 2,
                style = "padding:8px; border:1px solid #D8E2EA; text-align:justify;",
